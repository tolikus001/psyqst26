  ...
  if (req.method === 'POST') {
    ...
    req.on('end', async () => {
      ...
      if (pathname === '/api/lesson1-strategy') {
        ...
        const fallback = {
          trigger: 'Сработал подсознательный фильтр: как только возникает неопределенность или дистанция, внимание смещается с вопроса «Нужен ли мне этот человек?» на «Что мне сделать, чтобы меня выбрали?». Это автоматическое переключение отнимает вашу устойчивость.',
          ...
          action: 'Сделайте медленный выдох. Ваша задача на сегодня — отлепить реальные факты от тревожных фантазий ума. Остановите суету и спросите себя: «Если я сейчас промолчу, какую тревогу я пытаюсь заглушить?» Вернитесь в позицию выбирающей.'
        };
...
• **Ваш первый шаг заземления**: ${resData.action}
        `.trim();
